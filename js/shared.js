...
function cardHTML(d, { added = false, fav = false, actions = true } = {}) {
  return `<article class="dcard ${added ? 'added' : ''}" data-id="${d.id}" tabindex="0" aria-label="${esc(d.name)}, ${esc(d.country)}">
    <div class="dimg" style="${styleVars(d)}"><span class="cat">${d.cats[0]}</span>
      <button class="fav ${fav ? 'on' : ''}" data-fav="${d.id}" aria-label="${fav ? 'Remove from' : 'Add to'} favourites" aria-pressed="${fav}">${fav ? '♥' : '♡'}</button>
      <span class="emo" aria-hidden="true">${d.emoji}</span></div>
    <div class="dbody"><h3>${esc(d.name)} ${added ? '<span class="badge-in">In trip</span>' : ''}</h3>
      <div class="sub">${esc(d.country)} · Best: ${esc(d.season)}</div>
      <div class="price">~${money(dayCost(d))} / day <span class="muted">per person</span></div>
      ${actions ? `<div class="actions"><button class="btn sm ${added ? '' : 'primary'}" data-add="${d.id}">${added ? '✓ Added' : '＋ Add to trip'}</button><button class="btn sm" data-info="${d.id}">Details</button></div>` : ''}
    </div></article>`;
}
...
function showDest(d, { onAdd, onFav, added } = {}) {
  ...
  dlg.innerHTML = `<div class="dlg-img" style="${styleVars(d)}"><span aria-hidden="true">${d.emoji}</span><button class="icon-btn dlg-close" aria-label="Close">✕</button></div>
    <div class="dlg-body"><div><h2>${esc(d.name)}</h2><div class="muted">${esc(d.country)}</div></div>
      <p>${esc(d.blurb)}</p>
      <div class="chips">${d.cats.map(c => `<span class="chip">${c}</span>`).join('')}<span class="chip">📅 ${esc(d.season)}</span><span class="chip">💵 ~${money(dayCost(d))}/day</span></div>
      <div><strong>Top things to do</strong><ul class="dlg-list" style="margin-top:.5rem">${d.spots.map(s => `<li><span>${esc(s[0])}</span><b>${s[1] ? money(s[1]) : 'Free'}</b></li>`).join('')}</ul></div>
      <div class="muted">Daily estimate: stay ${money(d.cost.stay)} · food ${money(d.cost.food)} · transport ${money(d.cost.transport)}</div>
      <div class="row"><button class="btn primary" id="dlgAdd">${added ? '✓ In your trip' : '＋ Add to my trip'}</button><button class="btn" id="dlgFav">${fav ? '♥ Saved' : '♡ Save favourite'}</button></div></div>`;
  $('.dlg-close', dlg).onclick = () => dlg.close();
  $('#dlgAdd', dlg).onclick = () => { onAdd?.(d); dlg.close(); };
  $('#dlgFav', dlg).onclick = e => { const on = onFav ? onFav(d) : toggleFav(d.id); e.currentTarget.textContent = on ? '♥ Saved' : '♡ Save favourite'; };
  dlg.showModal();
}
